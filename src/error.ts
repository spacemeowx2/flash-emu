...
export function throwError (type: string, error: IErrorMessage, ...args: any[]) {
  // Errors.UndefinedVarError
  ...
  // type = native[type]
  if (type) {
    throw new Error(type + ' ' + error.message.replace(/%(\d)/, (_: any, i) => args[i - 1]))
  } else {
